...
2. Enable the Parameter Manager API from the "APIs & Services" menu of the Google Cloud Console. This can be done using the gcloud command line tool:
+
[source]
----
...
4. Using the https://console.cloud.google.com/security/parametermanager/parameters[Parameter Manager UI in Cloud Console], create a new parameter named `application-parameter` and set it to any value. Instructions for using the Parameter Manager UI can be found in the https://cloud.google.com/secret-manager/parameter-manager/docs/create-parameter#gcloud[https://cloud.google.com/parameter-manager/docs/quickstart][Parameter Manager documentation].
...
----
+
You will also see some web forms that allow you to create, read, and update parameters in Parameter Manager. This is done by using the `ParameterManagerTemplate`.
+
